...
class HeaderBagTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $bag = new HeaderBag();
        $bag->addCacheControlDirective('www', '#a');
        $this->assertTrue($bag->hasCacheControlDirective('www'));
        $this->assertEquals('#a', $bag->getCacheControlDirective('www'));
    }
    ...
    {
        $bag = new HeaderBag();
        $bag->addCacheControlDirective('www');

        $this->assertTrue($bag->hasCacheControlDirective('www'));
        $this->assertTrue($bag->getCacheControlDirective('www'));
        $this->assertEquals('www', $bag->get('cache-control'));

        $bag->addCacheControlDirective('max-age', 10);
        $this->assertTrue($bag->hasCacheControlDirective('max-age'));
        $this->assertEquals(10, $bag->getCacheControlDirective('max-age'));
        $this->assertEquals('max-age=10, www', $bag->get('cache-control'));
    ...
    {
        $bag = new HeaderBag(array('cache-control' => 'www, max-age=10'));
        $this->assertTrue($bag->hasCacheControlDirective('www'));
        $this->assertTrue($bag->getCacheControlDirective('www'));
        ...
        $this->assertEquals('max-age=10, www, s-maxage=100', $bag->get('cache-control'));
    }
    ...
    public function testCacheControlDirectiveOverrideWithReplace()
    {
        ...
        $bag->replace(array('cache-control' => 'www, max-age=10'));
        $this->assertTrue($bag->hasCacheControlDirective('www'));
        $this->assertTrue($bag->getCacheControlDirective('www'));
